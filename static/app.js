...
async function openDetail(id,initialTab="milestones"){
 ...
 const milestones=p.milestones.length?phaseNames.map(phaseName=>'<div class="phase-group"><div class="phase-heading"><strong>'+safe(phaseName)+'</strong><span>'+p.milestones.filter(m=>(m.phase_name||"Additional")===phaseName&&m.status==="Complete").length+'/'+p.milestones.filter(m=>(m.phase_name||"Additional")===phaseName).length+'</span></div>'+p.milestones.filter(m=>(m.phase_name||"Additional")===phaseName).map(m=>'<label class="milestone '+(m.status==="Complete"?"done":"")+'"><input type="checkbox" data-milestone="'+m.id+'" '+(m.status==="Complete"?"checked":"")+'><span class="milestone-name">'+safe(m.name)+'</span><span class="small">'+fmtDate(m.due_date)+'</span></label>').join("")+'</div>').join(""):'<div class="empty">No milestones yet.</div>';
 ...
 const work=workItems.length?workPhases.map(phaseName=>{
  ...
  return '<div class="work-phase"><div class="phase-heading"><strong>'+safe(phaseName)+'</strong><span>'+items.filter(item=>item.revio_work_item_id).length+'/'+items.length+' linked</span></div>'+items.map(item=>{
   ...
   const needsTaskId=item.item_type==="Task"&&!linked;
   const statusClass=linked?"linked":item.sync_error?"attention":"planned";
   return '<article class="work-item '+statusClass+'"><div class="work-item-main"><div class="work-item-title"><span class="work-type '+item.item_type.toLowerCase()+'">'+safe(item.item_type)+'</span><strong>'+safe(item.name)+'</strong></div><p>'+safe(item.description||"No work description entered.")+'</p><div class="work-meta"><span>Owner: '+safe(item.assignee_name||"Unassigned")+'</span><span>Estimate: '+(item.estimated_hours!=null?safe(item.estimated_hours)+"h":"Not set")+'</span><span>Status: '+safe(item.status)+'</span>'+(item.revio_item_id?'<span>Rev ID: '+safe(item.revio_item_id)+'</span>':'')+'</div>'+(item.sync_error?'<div class="work-error">'+safe(item.sync_error)+'</div>':'')+'</div>'+(needsTaskId?'<div class="work-link"><label>Rev PSA Calendar Item ID<input data-work-external="'+item.id+'" placeholder="Calendar item ID"></label><button class="secondary" data-link-work="'+item.id+'">Link Task</button></div>':'<div class="work-state">'+(linked?'✓ Linked to phase':item.item_type==="Ticket"?'Ready to create':'Needs scheduling')+'</div>')+'</article>'
  }).join("")+'</div>'
 ...
 '<div class="tab-panel hidden" data-detail-panel="work"><div class="section-box work-section"><div class="work-head"><div><h3>Phase Work</h3><p>Tickets are created and linked automatically. Schedule tasks in Rev PSA, then paste the Calendar Item ID to link them.</p></div>'+(p.revio_project_id?'<button class="primary" id="sync-revio-work">Create / Sync Rev PSA Work</button>':'<span class="work-notice">Create this project in Rev PSA first.</span>')+'</div><div class="work-summary"><span>'+workItems.filter(item=>item.item_type==="Ticket").length+' tickets</span><span>'+workItems.filter(item=>item.item_type==="Task").length+' tasks</span><span>'+workItems.reduce((sum,item)=>sum+(Number(item.estimated_hours)||0),0).toFixed(1)+' planned hours</span></div>'+work+'</div></div>'+
 ...
 if(createRevio)createRevio.onclick=async()=>{createRevio.disabled=true;createRevio.textContent="Creating…";try{const result=await api("/api/projects/"+id+"/revio/create",{method:"POST"});await refresh();openDetail(id);const work=result.work||{};toast("Rev PSA Project "+result.revio_project_id+" created with "+result.phases_created+" phases, "+result.milestones_created+" milestones, and "+(work.linked||0)+" linked work item(s)"+(work.pending_tasks?" · "+work.pending_tasks+" task(s) need scheduling":""))}catch(e){await refresh();openDetail(id);toast("Rev PSA creation failed: "+e.message)}};
 const syncPhases=$("#sync-revio-phases");
 if(syncPhases)syncPhases.onclick=async()=>{syncPhases.disabled=true;syncPhases.textContent="Syncing phases…";try{const result=await api("/api/projects/"+id+"/revio/sync-phases",{method:"POST"});await refresh();openDetail(id);const work=result.work||{};toast("Rev PSA phases synced: "+result.phases_created+" created, "+result.milestones_moved+" milestones organized, "+(work.linked||0)+" work item(s) linked"+(work.pending_tasks?" · "+work.pending_tasks+" task(s) need scheduling":""))}catch(e){await refresh();openDetail(id);toast("Rev PSA phase sync failed: "+e.message)}};
 const syncWork=$("#sync-revio-work");
 if(syncWork)syncWork.onclick=async()=>{syncWork.disabled=true;syncWork.innerHTML='<span class="button-spinner"></span> Syncing work…';try{const result=await api("/api/projects/"+id+"/revio/sync-work",{method:"POST"});await refresh();openDetail(id,"work");const parts=[result.linked+" linked"];if(result.pending_tasks)parts.push(result.pending_tasks+" task(s) need scheduling");if(result.failed)parts.push(result.failed+" need attention");toast("Rev PSA work synced: "+parts.join(", "))}catch(e){await refresh();openDetail(id,"work");toast("Rev PSA work sync failed: "+e.message)}};
 Array.from(document.querySelectorAll("[data-link-work]")).forEach(button=>button.onclick=async()=>{const itemId=button.dataset.linkWork,input=$("[data-work-external='"+itemId+"']"),externalId=input?.value.trim();if(!externalId){toast("Enter the Rev PSA Calendar Item ID first");input?.focus();return}button.disabled=true;button.textContent="Linking…";try{await api("/api/work-items/"+itemId+"/link",{method:"POST",body:JSON.stringify({external_id:externalId})});await refresh();openDetail(id,"work");toast("Task linked to its Rev PSA phase")}catch(e){button.disabled=false;button.textContent="Link Task";toast("Task link failed: "+e.message)}});
 ...
 try{
  ...
  if(createInRevio&&!saved.revio_project_id){
   ...
   try{
    ...
    const work=result.work||{};revioMessage=" Rev PSA Project "+result.revio_project_id+" created with "+result.phases_created+" phases, "+result.milestones_created+" milestones, and "+(work.linked||0)+" linked work item(s)."+(work.pending_tasks?" "+work.pending_tasks+" scheduled task(s) still need Calendar Item IDs.":"")
